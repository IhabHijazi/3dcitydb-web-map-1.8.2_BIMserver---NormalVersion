...
var DataSourceController = /** @class */ (function () {
    ...
    //added
    DataSourceController.prototype.fetchDataPromise = async function (gmlIds) {
        var scope = this;
        let resultArray = gmlIds.map((gmlId)=>{
             return scope._dataSource.queryUsingIdPromise(gmlId)
            .then((unstructeredResult)=>scope._dataSource.responseToKvp(unstructeredResult))
            .catch((error)=>console.log(error))
        });
        let resolvedResultArray = await Promise.all(resultArray);
        return resolvedResultArray;
    };
