...
// added
GoogleSheets.prototype.queryUsingIdPromise = async function (id) {
    let result = await this.queryUsingSqlPromise("SELECT * WHERE A='" + id + "'");
    return result;
};
// added
GoogleSheets.prototype.queryUsingSqlPromise = function (sql) {
    // TODO handle limit
    let spreadsheetId = this._spreadsheetId;
    return new Promise(function (resolve, reject) {
        var baseUrl = "https://docs.google.com/spreadsheets/d/";
        var xmlHttp = new XMLHttpRequest();
        xmlHttp.open("GET", baseUrl + spreadsheetId + "/gviz/tq?tq=" + encodeURI(sql)); // true for asynchronous
        xmlHttp.onload = function () {
            if (this.status >= 200 && this.status < 300) {
                resolve(JSON.parse(xmlHttp.response.replace("/*O_o*/", "").replace(/(google\.visualization\.Query\.setResponse\(|\);$)/g, "")));
                // The response is in JSON but contains the following string:
                // "/*O_o*/google.visualization.Query.setResponse({status:ok, ...})"
                // https://developers.google.com/chart/interactive/docs/dev/implementing_data_source#jsondatatable
                // The Google Visualization API is used here for querying data from Google Spreadsheets
